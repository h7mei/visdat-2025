...
// Import correlation matrix module
import { createCorrelationMatrix } from "./correlation-matrix.js";
...
// Create correlation matrix container
const correlationMatrixSvg = d3
...
d3.csv("sample.csv").then((data) => {
  ...
  data.forEach((d) => {
    d.rank = +d.rank || 0;
    d.averageRating = +d.averageRating || 0;
    d.startYear = +d.startYear || 0;
    d.runtimeMinutes = +d.runtimeMinutes || 0;
  ...
  data = data.filter(
    (d) => !isNaN(d.averageRating) && !isNaN(d.numVotes) && !isNaN(d.runtimeMinutes) && !isNaN(d.startYear)
  );
  ...
  d3.select("#genre-filter").on("change", function () {
    ...
    const filteredData = selectedGenre === "all" ? data : data.filter((d) => d.genres.includes(selectedGenre));
...
function updateBarChart(data) {
  ...
  // Update SVG height
  d3.select("#bar-chart svg").attr("height", chartHeight);
  ...
  // Add grid
  barChartSvg.append("g").attr("class", "grid").call(d3.axisLeft(y).tickSize(-width).tickFormat(""));
...
function updateScatterPlot(data) {
  ...
  // Add axes
  scatterPlotSvg.append("g").attr("transform", `translate(0,${height})`).call(d3.axisBottom(x));

  scatterPlotSvg.append("g").call(d3.axisLeft(y));

  // Add grid
  scatterPlotSvg.append("g").attr("class", "grid").call(d3.axisLeft(y).tickSize(-width).tickFormat(""));
...
function updatePieChart(data) {
  ...
  // Create pie chart
  const arcs = pieChartSvg.selectAll(".arc").data(pie(genreData)).enter().append("g").attr("class", "arc");
  ...
  // Add legend
  const legend = pieChartSvg.append("g").attr("transform", `translate(${radius + 20}, -${height / 2})`);
